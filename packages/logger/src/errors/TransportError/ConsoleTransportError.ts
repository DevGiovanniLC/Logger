...
export class ConsoleTransportError extends InternalError {
    ...
    constructor(message: string, public code = "ConsoleTransport Error", status = 0) {
        super(message);
...
export function requireConsole(boundary: Function | Object, missing?: string[]): never {
    const detail = missing && missing.length > 0 ? ` Missing methods: ${missing.join(", ")}.` : "";
    return errorThrower(
        boundary,
